  ...
  const getCurrentRgb = () => {
    ...
    if (deviceState.color && typeof deviceState.color === 'object') {
      ...
      if ('r' in deviceState.color && 'g' in deviceState.color && 'b' in deviceState.color) {
        // Use nullish coalescing to handle undefined/null, and fallback for NaN
        const r = deviceState.color.r ?? 255;
        const g = deviceState.color.g ?? 255;
        const b = deviceState.color.b ?? 255;
        const rgb = {
          r: Number.isNaN(Number(r)) ? 255 : Number(r),
          g: Number.isNaN(Number(g)) ? 255 : Number(g),
          b: Number.isNaN(Number(b)) ? 255 : Number(b)
        };
      ...
      // Check if it's HSV format
        ...
        const h = deviceState.color.h ?? 0;
        const s = deviceState.color.s ?? 1;
        const v = deviceState.color.v ?? 1;
        const rgb = hsvToRgb(
          Number.isNaN(Number(h)) ? 0 : Number(h),
          Number.isNaN(Number(s)) ? 1 : Number(s),
          Number.isNaN(Number(v)) ? 1 : Number(v)
        );
