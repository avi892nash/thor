...
class WizLight extends EventEmitter {
  ...
  async updateProperties(payload: UpdatePropertiesPayload): Promise<boolean> {
    ...
    // Handle scene (takes highest priority - scenes override color/temp settings)
    if (payload.sceneId !== undefined) {
      // Always ensure state is ON when setting a scene
      params['state'] = true;
      params['sceneId'] = payload.sceneId;
      stateUpdate.sceneId = payload.sceneId;
      stateUpdate.state = true;

      // Speed is only relevant for dynamic scenes (not Rhythm mode 1000)
      if (payload.speed !== undefined && payload.sceneId !== 1000) {
        params['speed'] = clamp(payload.speed, 10, 200);
        stateUpdate.speed = payload.speed;
      }

      // Dimming can be applied with scenes (including Rhythm)
      if (payload.brightness !== undefined) {
        params['dimming'] = clamp(payload.brightness, 10, 100);
        stateUpdate.brightness = payload.brightness;
      }
    }
    // Handle color (takes priority over individual brightness/temperature)
    else if (payload.color && payload.color.r !== undefined && payload.color.g !== undefined && payload.color.b !== undefined) {
      ...
      params['dimming'] = clamp(brightness, 10, 100);
    ...
    // Handle cold/warm white mix
    else if (payload.coldWhite !== undefined || payload.warmWhite !== undefined) {
      if (payload.coldWhite !== undefined) {
        params['c'] = clamp(payload.coldWhite, 0, 255);
        stateUpdate.coldWhite = payload.coldWhite;
      }
      if (payload.warmWhite !== undefined) {
        params['w'] = clamp(payload.warmWhite, 0, 255);
        stateUpdate.warmWhite = payload.warmWhite;
      }
      if (payload.brightness !== undefined) {
        params['dimming'] = clamp(payload.brightness, 10, 100);
        stateUpdate.brightness = payload.brightness;
  ...
  async setScene(sceneId: number, speed?: number, brightness?: number): Promise<boolean> {
    const params: Record<string, unknown> = { sceneId };
    const stateUpdate: Partial<LightState> = { sceneId };

    if (speed !== undefined) {
      params['speed'] = Math.max(10, Math.min(200, speed));
      stateUpdate.speed = speed;
    }

    if (brightness !== undefined) {
      params['dimming'] = Math.max(10, Math.min(100, brightness));
      stateUpdate.brightness = brightness;
    }
